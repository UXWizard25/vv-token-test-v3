/**
 * Style Dictionary Configuration (v4) - uses pipeline.config.js
 *
 ...
 * @returns {String} - Style Dictionary version (e.g., '4.2.0')
 */
 ...
 * @param {String} options.commentStyle - Comment style: 'block' | 'line'
 * @param {String} [options.platform] - Platform identifier for docs URL: 'css' | 'scss' | 'js' | 'ios' | 'android'
...
function getBrandAttribute(modeType, filePath = '') {
  ...
  return DATA_ATTRS.contentBrand;
}
...
const cssEffectClassesFormat = ({ dictionary, options }) => {
  ...
  const attrSelector = `[${brandAttr}="${brandLowercase}"][${DATA_ATTRS.theme}="${colorMode}"]`;
...
const cssThemedVariablesFormat = ({ dictionary, options, file }) => {
  ...
  if (brand && mode) {
    ...
    const dataMode = modeType === 'theme' ? DATA_ATTRS.theme :
                     modeType === 'density' ? DATA_ATTRS.density : `data-${modeType}`;
...
const swiftuiEnumsFormat = ({ dictionary, options, file }) => {
  ...
  // Generate SizeClass enum cases from config
  const sizeClassCases = Object.keys(pipelineConfig.platforms.ios.sizeClasses)
    .map(sc => `    case ${sc}`)
    .join('\n');
...
/// Available brands in the ${pipelineConfig.identity.name}
...
const swiftuiComponentTokensFormat = ({ dictionary, options, file }) => {
  ...
  let output = generateFileHeader({
    ...
    platform: 'ios',
    brand: brandPascal,
    context: `Component ${componentPascal} (${tokenType})`
  });
...
const composeSemanticColorsFormat = ({ dictionary, options, file }) => {
  ...
  let output = generateFileHeader({
    ...
    platform: 'android',
    brand: brandPascal,
    context: `SemanticColors ${modePascal}`
  });
...
import androidx.compose.ui.graphics.Color
import ${ANDROID_PKG}.shared.DesignColorScheme
...
const composeSpacingFormat = ({ dictionary, options, file }) => {
  ...
  if (modeType === 'sizeclass') {
    imports.push(`import ${ANDROID_PKG}.shared.DesignSizingScheme`);
  }
  if (modeType === 'density') {
    imports.push(`import ${ANDROID_PKG}.shared.DesignDensityScheme`);
  }
  ...
  let output = generateFileHeader({
    ...
    platform: 'android',
    brand: brandPascal,
    context: mode ? `Spacing ${modeType}: ${modePascal}` : 'Spacing'
  });
...
const composeComponentTokensFormat = ({ dictionary, options, file }) => {
  ...
  let output = generateFileHeader({
    ...
    platform: 'android',
    brand: brandPascal,
    context: `Component ${componentName}${mode ? ` (${tokenType}: ${modePascal})` : ''}`
  });
...
const composeTypographySchemeFormat = ({ dictionary, options, file }) => {
  ...
  let output = generateFileHeader({
    ...
    platform: 'android',
    brand: brandPascal,
    context: `TypographyScheme ${sizeClassPascal}`
  });
...
import ${ANDROID_PKG}.shared.DesignTypographyScheme
import ${ANDROID_PKG}.shared.DesignTextStyle
import ${ANDROID_PKG}.shared.DesignTextCase
...
const composeTypographyFormat = ({ dictionary, options, file }) => {
  ...
  let output = generateFileHeader({
    ...
    platform: 'android',
    brand: brandPascal,
    context: `Typography${mode ? ` ${modePascal}` : ''}${componentName ? ` (${componentName})` : ''}`
  });
...
const composeEffectsFormat = ({ dictionary, options, file }) => {
  ...
  let output = generateFileHeader({
    ...
    commentStyle: 'block',
    platform: 'android',
    context: `${isLight ? 'Light' : 'Dark'} Mode Effects (Shadows)\n * Brand-independent: shadows only depend on light/dark mode`
  });
...
const composeComponentEffectsFormat = ({ dictionary, options, file }) => {
  ...
  let output = generateFileHeader({
    ...
    platform: 'android',
    brand: brand,
    context: `Component: ${componentName} | Brand: ${brand}\n * ${isLight ? 'Light' : 'Dark'} Mode Effects (Shadows)`
  });

  output += `
package ${ANDROID_PKG}.brands.${brand.toLowerCase()}.components.${componentName.toLowerCase()}

import androidx.compose.ui.graphics.Color
import ${ANDROID_PKG}.shared.DropShadow
import ${ANDROID_PKG}.shared.ShadowStyle
