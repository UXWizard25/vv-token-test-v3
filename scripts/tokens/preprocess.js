 ...
 * @param {object} context - { brandName, brandModeId: BRANDS[figmaName], breakpointModeId, colorModeModeId }
...
function processBrandSpecificTokens(collections, aliasLookup) {
  ...
  const colorBrandsSet = new Set(COLOR_BRANDS);
  const outputs = {};
  ...
  collections.forEach(collection => {
    ...
    Object.entries(BRAND_INFO).forEach(([brandKey, { figmaName }]) => {
      // Only process brands that are in the configured brand list
      if (!ALL_BRANDS.includes(brandKey)) return;
      ...
      collection.variables.forEach(variable => {
        ...
        if (modeValue !== undefined && modeValue !== null) {
          ...
          if (modeValue.type === 'VARIABLE_ALIAS') {
            ...
            const context = { brandName: figmaName, brandModeId: BRANDS[figmaName] };
            processedValue = resolveAliasWithContext(modeValue.id, aliasLookup, context, new Set(), collections);
          } else {
            processedValue = processDirectValue(modeValue, variable.resolvedType, variable.name);
          }
...
function processEffectTokens(effectStyles, aliasLookup, collections) {
  ...
  Object.entries(BRAND_INFO).forEach(([brandKey, { figmaName }]) => {
    ...
    if (!COLOR_BRANDS.includes(brandKey)) {
