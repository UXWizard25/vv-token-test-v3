...
async function buildOptimizedJSOutput() {
  ...
  let primitivesOutput = JS_FILE_HEADER;
  ...
  for (const brand of BRANDS) {
    ...
    if (hasColors) {
      ...
      let output = JS_FILE_HEADER + `// Brand: ${brand}\n\n`;
    ...
    let spacingOutput = JS_FILE_HEADER + `// Brand: ${brand}\n\n`;
    ...
    let typoOutput = JS_FILE_HEADER + `// Brand: ${brand}\n\n`;
    ...
    if (fs.existsSync(componentsDir)) {
      ...
      for (const comp of componentNames) {
        ...
        let compOutput = JS_FILE_HEADER + `// Component: ${comp}\n// Brand: ${brand}\n\n`;
      ...
      if (builtComponents.length > 0) {
        let compIdx = JS_FILE_HEADER + `// Components for ${brand}\n\n`;
    ...
    // Brand index
    let brandIdx = JS_FILE_HEADER + `// ${brand.toUpperCase()} Brand\n\n`;
