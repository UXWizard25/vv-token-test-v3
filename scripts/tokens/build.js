...
/**
 * Loads the Breakpoint × Density matrix generated by preprocess.js
 * This matrix contains fully resolved values (ID-based resolution)
 *
 * @returns {Object} Matrix with tokens and their Breakpoint × Density values
 */
function loadBreakpointDensityMatrix() {
  const matrixPath = path.join(TOKENS_DIR, 'shared', 'breakpoint-density-matrix.json');

  if (!fs.existsSync(matrixPath)) {
    console.warn(`Breakpoint × Density matrix not found: ${matrixPath}`);
    return {};
  }
...
function generateiOSDensityResolvers() {
  ...
  let output = `    // MARK: - Density-Aware Spacing (Auto-generated from Breakpoint × Density Matrix)
    //
    // Values resolved at build-time using ID-based cross-collection resolution.
...
async function main() {
  ...
  // Note: Documentation files are maintained in their canonical locations:
  // - CSS: packages/tokens/docs/css.md (included via package.json files array)
  // - JS: packages/tokens/docs/js.md (currently disabled)
  // - Android: packages/tokens-android/docs/USAGE.md
  // - iOS: packages/tokens-ios/Documentation/USAGE.md
  // No copying to dist/ needed - docs/ is the standard npm convention for documentation.
