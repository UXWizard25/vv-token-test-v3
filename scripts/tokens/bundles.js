...
function generateHeader(brand, bundleType, customDescription = null) {
  ...
  const typeDescriptions = {
    'primitives': 'Primitives (Base values: colors, space, size, fonts)',
    'color-semantics': 'Color Semantics (Semantic colors + Effect variables)',
    'sizing-semantics': 'Sizing Semantics (Typography + Density + Breakpoints)',
    ...
    'full': 'Full Bundle (All tokens, self-contained)',
    'full-utilities': 'Full Utilities Bundle (All classes)',
  };

  // Dependency hints for split-file loading
  const requiresMap = {
    'primitives': null,
    'color-semantics': ['shared/primitives.css'],
    'sizing-semantics': ['shared/primitives.css'],
    'utilities': ['shared/primitives.css', 'color-semantics.css', 'sizing-semantics.css'],
    'component': ['shared/primitives.css', 'color-semantics.css', 'sizing-semantics.css'],
    'component-utilities': ['shared/primitives.css', 'color-semantics.css', 'sizing-semantics.css'],
    'full': null,
    'full-utilities': [`bundles/${brand || 'brand'}.css`],
  };

  const description = customDescription || typeDescriptions[bundleType] || 'CSS Bundle';
  const requires = requiresMap[bundleType];
...
function cleanupLegacyStructure() {
  console.log('\n🧹 Cleaning up legacy/intermediate files...');
  ...
  if (removedDirs.length > 0) {
    console.log(`   ✅ Removed: ${removedDirs.join(', ')}`);
  } else {
...
async function buildSharedPrimitives() {
  ...
  if (primitiveFiles.length === 0) {
    console.log('  ⚠️  No primitive files found');
    return '';
  }
  ...
  console.log(`  ✅ primitives.css (${primitiveFiles.length} sources, ${getFileSize(content)} KB)`);
...
async function buildBrandColors(brand) {
  ...
  let content = generateHeader(brand, 'color-semantics');
...
async function buildBrandSizing(brand) {
  ...
  let content = generateHeader(brand, 'sizing-semantics');
